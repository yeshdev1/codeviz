...
- **`system-map.html`** — the centerpiece: a **semantic-zoom atlas** where **every box is a real system and every arrow is a real call**, arranged in colour-coded layers. Scroll (or use the Continents/Services/Detail pills) to zoom from the layer overview down to per-system detail; click a system to focus its neighbourhood; run the **guided tour** to walk a real request hop-by-hop with a comet, grad-level narration and jargon chips. Self-contained, offline. *Spend the most effort here.* It carries a top nav to the two companion pages.
- **`data-model.html`** — for each datastore, three stacked zones: **context** (what it holds + why), then the **ER diagram** itself (tables + relationship lines drawn with crow's-foot *many→one* cardinality), then a **comprehensive, new-grad-level walkthrough** of how the tables relate. Built from the `DATAMODEL` block by **`assets/render-pages.js`** (author `context`/`explain` per store; the walkthrough is auto-generated from the foreign keys if you omit `explain`). Clicking a datastore on the map deep-links here.
- **`scenarios.html`** — each guided-tour flow as a **collapsible, beginner-friendly walkthrough** (collapsed by default): every step is shown against the **whole-system schematic** (the active hop lit, past dimmed, future dashed) beside an **entry-level explanation** that assumes no prior knowledge. Built from `SCENARIOS` (its optional `intro`/`lesson`/`callout`/`learn` fields) by **`render-pages.js`**.
- **`schema.html`** *(legacy/alternate)* — a simpler data model grouped by domain on `harness.css`; the interactive `data-model.html` above is the modern data view.
...
- `assets/system-map.template.html` — the complete, runnable **atlas engine** with a generic example and clearly marked `@@ REPLACE @@` data blocks (`NODES`, `DOMAINS`, `EDGES`, `EDGE_DETAIL`, `SCENARIOS`, `DATAMODEL`, `HEALTH_SCENARIOS`). Self-contained (no CSS/CDN deps). Adapt the data; don't rewrite the engine.
- `assets/render-pages.js` — a project-agnostic, zero-dependency Node script that reads the data + theme straight out of a generated `system-map.html` and emits the companion `data-model.html` + `scenarios.html` (collapsible) and adds the nav. Run it in Deliver.
...
1. **Nodes** — enumerate the real systems: client apps · the API/services · datastores · cache · queue/workers · 3rd parties · CDN/storage. For each fill `{ tier, w, h, label, sub, status, about, resp:[] }`. `tier` is the layer index (0 = clients at top, then API, data, workers, 3rd-parties). `status` ∈ `built` (code exists today) · `partial` · `planned`. `about` + `resp[]` power the hover card — write them for the reader, not as notes. Optional `health:{ state:'up|degraded|down', p99, err, note }` feeds the Health overlay (illustrative only — omit unless you have a point to make; un-annotated nodes show healthy). **Cite the file(s)** each node maps to in your working notes. **No `x, y`** — the engine lays nodes out by layer automatically and guarantees no overlaps.
...
### Deliver
**Generate the companion pages:** once `system-map.html` is written, run `node <skill>/assets/render-pages.js <output-dir>` (default `docs/onboarding`). It reads the map's `DATAMODEL` + `SCENARIOS`, emits **`data-model.html`** (full-page ER per store) and **`scenarios.html`** (collapsible beginner walkthroughs), and adds the top nav + datastore deep-links. Zero dependencies — plain Node. For a real onboarding scenarios page, author the optional beginner fields on `SCENARIOS` (`intro` per scenario; `lesson`/`callout`/`learn` per step) — without them the page falls back to the terse tour text.
...
9. **Health overlay + scenarios** — a **Structure | Health** toggle recolours nodes & edges by health (up = green · degraded = amber · down = red), shows per-node p99/error metrics, and lights up the unhealthy paths. **`HEALTH_SCENARIOS`** are named snapshots the reader switches between (e.g. *Sample incident · All healthy · Database degraded (cascade)*); `base:'up'` resets all healthy, `states` override per node. Toggling Health shows an **at-a-glance summary** — counts plus a full one-line explanation per affected system (state · metrics · the `note`), each clickable to fly there — so write every `note` as a **complete, self-contained explanation** (symptom + likely cause, e.g. "degraded — calls Postgres (down) via sync SQL"), since it's read inline, not just on hover. Modeled scenarios are **ILLUSTRATIVE** (permanent "modeled — not observed" badge; codeviz reads static code, not live metrics). Users model their **own** what-ifs with **/codeviz-scenario** (e.g. "database outage") — it computes the blast radius up the call graph and appends a modeled scenario chip. For **real** health, **/codeviz-health** snapshots local Docker (state/uptime/restarts/healthchecks) into a `source:'observed'` scenario — a point-in-time snapshot, not continuous monitoring. Per-node default still comes from optional `NODES[].health`.
10. **Accessibility & motion** — `prefers-reduced-motion` swaps animated flow for static two-colour direction chevrons, disables camera tweens + auto-advance, and freezes the comet; an `aria-live` region announces focus, each tour leg (request out / response back), and the health state. Verify with the headless checks, not a "we followed WCAG" claim.
