...
- **`data-model.html`** — every datastore's **entity-relationship diagram**, full-page (tables, keys, foreign-key connectors as drawn lines) plus a **Joins & retrieval** section. Built from the `DATAMODEL` block by **`assets/render-pages.js`**; clicking a datastore on the map deep-links here.
