...
- **`system-map.html`** — the centerpiece: a **semantic-zoom atlas** where **every box is a real system and every arrow is a real call**, arranged in colour-coded layers. Scroll (or use the Continents/Services/Detail pills) to zoom from the layer overview down to per-system detail; click a system to focus its neighbourhood; run the **guided tour** to walk a real request hop-by-hop with a comet, grad-level narration and jargon chips. The tour is **diggable** — a Dig-depth dial (**Overview** = just the hop → **Walkthrough** = text+detail, the recommended default → **Deep dive** = the full beginner lesson, plain-English callout and jargon), with a **gate** past the recommended level ("more than you need to follow the flow — dig in anyway?"). This is where scenarios live; there is no separate scenarios page. Self-contained, offline. *Spend the most effort here.* It carries a top nav to the data-model page.
- **`data-model.html`** — for each datastore, three stacked zones: **context** (what it holds + why), then the **ER diagram** itself (tables auto-laid-out by dependency — roots left, junctions right — with crow's-foot *many→one* cardinality landing on the target PK; **hover a table to spotlight just its relationships**; junction/hub tables auto-badged), then a **comprehensive, new-grad-level walkthrough** of how the tables relate. A per-store **Dig depth** dial (Entities→Keys→Columns→Everything) opens at a grain-derived **recommended** level and **gates** anything deeper ("reference noise, not insight — dig in anyway?"). Built from the `DATAMODEL` block by **`assets/render-pages.js`** (author `context`/`explain` per store; the walkthrough is auto-generated from the foreign keys if you omit `explain`). Clicking a datastore on the map deep-links here.
...
- `assets/render-pages.js` — a project-agnostic, zero-dependency Node script that reads the data + theme straight out of a generated `system-map.html` and emits the companion `data-model.html` and adds the nav. Run it in Deliver. (Scenarios are not a page — they're the diggable map tour.)
