...
5. **Render** — copy `assets/system-map.template.html` into the output dir as `system-map.html` (it is fully self-contained — no `harness.css`/`theme.css`, no CDN); replace the five `@@ REPLACE @@` blocks (`NODES`, `DOMAINS`, `EDGES`, `EDGE_DETAIL`, `SCENARIOS`) and `@@PROJECT@@`. *(The atlas ships its own dark palette inline; per-project theming of the canvas is a known follow-up — the other pages still use `theme.css`.)*
...
- **Scenario (guided tour):** `{ id, label, steps:[{from,to,payload,text,detail,terms}] }`. `text` = what happens; `detail` = why (teach a grad; `<b>` allowed); `terms` = `[[name, definition], …]` → jargon chips. Each step flies the camera to frame the hop and runs a comet on the `from→to` edge, so **that edge must exist**. `SCENARIOS[0]` is offered first. Decompose hops for granularity.
...
7. **Guided tour (onboarding scrollytelling)** — the comet walks a real flow hop-by-hop **as a round trip** (request out in blue, then response back in orange) while the camera flies to frame the hop; each step carries a grad-level explanation + jargon chips. Animated transitions preserve object constancy (Heer & Robertson).
