...
4. **Render** — copy `assets/system-map.template.html` + `assets/harness.css` into the output dir; replace the four `@@ REPLACE @@` blocks (`NODES`, `EDGES`, `DETAIL`, `SCENARIOS`) and `@@PROJECT@@`. Set each node's `tier` (0 top → bottom) and let auto-layout place them; the engine computes positions with elkjs and guarantees no overlaps. (Hand-placed `{ x, y }` on the 880×510 grid still works if you want to override; open any page with `?layout=elk` to force auto-layout on a hand-placed map.)
