...
(function () {
  ...
  function renderError(msg) { card.innerHTML = '<h4><span class="cvx-dot"></span>Couldn’t enable</h4><p>' + esc(msg || 'Failed to load the model.') + '</p>'; }
  function enable() {
    if (state === 'loading' || state === 'ready') return;     // guard: don't start a second model load
    ...
    loadEngine(function (p) { if (state === 'loading') renderLoading(p); }).then(function () {
      ...
      try { localStorage.removeItem(PREF); } catch (e) {}   // a failed/unsupported load must not auto-retry & re-error on every reload
      setPill(state === 'unsupported' ? 'unsupported' : 'error'); renderError(err && err.message);
    });
  }
  ...
  function onSelectionEnd(ev) {
    ...
    if (state !== 'ready') { card.classList.remove('cvx-hidden'); if (state !== 'loading') renderOffer(); return; }   // nudge to enable, but don't clobber the download progress UI mid-load
